...
int main(int argc, char *argv[])
{
   ...
   long unsigned int *key_schedule = (long unsigned int *)malloc(sizeof(long unsigned int) * 60);
   unsigned int idx;

   /* Taking file input for encryption */
   FILE *fp;
   char hex[65];
   int i, used = 0;
   fp = fopen(argv[1], "r");
   if (fp == NULL) {
      printf("File read failed. Exiting!\n");
      exit(-1);
   }
   fgets(hex, sizeof(hex), fp);
   for (i = 0; i < 16; i++) {
      sscanf(hex+used, "%4hhx", &plaintext[i]);
      used += 4;
   }
   ...
   /* Taking file input for decryption */
   used = 0;
   fp = fopen(argv[2], "r");
   if (fp == NULL) {
      printf("File read failed. Exiting!\n");
   ...
   fgets(hex, sizeof(hex), fp);
   for (i = 0; i < 16; i++) {
      sscanf(hex+used, "%4hhx", &ciphertext[i]);
      used += 4;
   }
